<template>
  <div class="item">
    <div>
      ...
      <h6>{{item.name}}</h6>
      <h3>{{item.shop_info.spu_sub_title}}</h3>
      <div class="params-colors">
        <ul class="colors-list">
          <li
            v-for="(sku,index) in item.sku_info"
            :key="index"
          >
            <a
              href="javascript:;"
              ...
              :data-index="index"
              :title="sku.spec_json[0].show_name"
              :class="{'active':index==itemIndex}"
            ><img :src="'http://img01.smartisanos.cn/'+ sku.spec_json[0].image +'/20X20.jpg'"></a>
          </li>
        </ul>
      </div>
